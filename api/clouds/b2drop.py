...
B2DROP_DOMAIN = 'b2drop.eudat.eu'
B2DROP_PATH = 'remote.php/webdav'
...
def check_b2drop_credentials(provider):
    webdav = _connect(provider)
    webdav.ls('/')


def retrieve_file_from_b2drop(datafile, provider):
    webdav = _connect(provider)
...
def create_folder_on_b2drop(folder, provider):
    webdav = _connect(provider)
...
def delete_file_from_b2drop(provider, storage_key):
    webdav = _connect(provider)
...
def resync_from_b2drop(provider):
    webdav = _connect(provider)
    ...
    # Fetch the data
    remote_data = webdav.ls('/', depth=easywebdav.DEPTH_INFINITY)
    ...
    for f in webdav.ls('/', depth=easywebdav.DEPTH_INFINITY):
...
def upload_file_to_b2drop(datafile, provider, tempfile_path):
    webdav = _connect(provider)
